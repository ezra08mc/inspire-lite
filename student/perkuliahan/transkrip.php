...
try {
    $stmt = $pdo->prepare(
        "SELECT g.*, s.course_name, s.sks, s.semester FROM academic_grades g JOIN subjects s ON g.course_code = s.course_code WHERE g.nim = ? ORDER BY s.semester ASC, s.course_code ASC",
    );
    $stmt->execute([$nim]);
...
$ipk = $total_sks > 0 ? round($total_grade_points / $total_sks, 2) : "0.00";

$base_path = "../../";
$page_title = "Transkrip Nilai Kumulatif - INSPIRE Lite";
    ...
    <main class="dashboard-viewport">
        ...
        <div class="content-card">
            <div class="card-top">
                ...
                <button onclick="window.print()" class="trigger-btn" style="padding: 6px 12px; font-size: 0.75rem; background-color: #6b7280; color: white; border: none;">Cetak Transkrip</button>
            </div>
            <div class="agenda-table-wrapper" style="margin: 16px;">
                <div class="agenda-table-header">
                    ...
                    <span class="col-head">SEM</span>
                    <span class="col-head">NILAI</span>
                    <span class="col-head">BOBOT</span>
                </div>
                <div class="agenda-rows-stack">
                    <?php if (empty($grades)): ?>
                        <div class="empty-fallback-text border-box-pad">Data transkrip belum tersedia.</div>
                    <?php else: ?>
                        <?php foreach ($grades as $g): ?>
                            <div class="agenda-table-row">
                                ...
                                <div class="col-cell" style="flex: 0.3; color: #6b7280;">
