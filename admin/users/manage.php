...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"])) {
    try {
        if ($_POST["action"] === "delete") {
            ...
            $pdo->prepare("UPDATE users SET role = ? WHERE id = ?")->execute([
                $new_role,
                $uid,
            ]);
            ...
            if ($new_role === "student") {
                $pdo->prepare(
                    "INSERT INTO students (user_id, nim, first_name, last_name, birth_date, study_program, cohort) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE nim=?, first_name=?, last_name=?, birth_date=?, study_program=?, cohort=?",
                )->execute([
                    $uid,
                    $_POST["nim"],
                    $fn,
            ...
            } elseif ($new_role === "lecturer") {
                $pdo->prepare(
                    "INSERT INTO lecturers (user_id, nip, first_name, last_name, birth_date, degree, expertise) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE nip=?, first_name=?, last_name=?, birth_date=?, degree=?, expertise=?",
                )->execute([
                    $uid,
                    $_POST["nip"],
                    $fn,
                ...
                $pdo->prepare(
                    "DELETE FROM lecturer_courses WHERE nip=?",
                )->execute([$_POST["nip"]]);
                if (
                    !empty($_POST["assigned_courses"]) &&
                    is_array($_POST["assigned_courses"])
                ) {
                    $stmt_lc = $pdo->prepare(
                        "INSERT INTO lecturer_courses (nip, course_code) VALUES (?, ?)",
                    );
                    foreach ($_POST["assigned_courses"] as $cc) {
                        $stmt_lc->execute([$_POST["nip"], $cc]);
                    }
                }
            } elseif ($new_role === "staff") {
                $pdo->prepare(
                    "INSERT INTO staff (user_id, staff_id, first_name, last_name, birth_date, division, position) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE staff_id=?, first_name=?, last_name=?, birth_date=?, division=?, position=?",
                )->execute([
                    $uid,
                    $_POST["staff_id"],
                    $fn,
                ...
                $pdo->prepare(
                    "INSERT INTO admins (user_id, admin_id, first_name, last_name) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE first_name=?, last_name=?",
                )->execute([$uid, $_POST["admin_id"], $fn, $ln, $fn, $ln]);
            }
            $pdo->commit();
...
try {
    $all_subjects = $pdo
        ->query(
            "SELECT course_code, course_name FROM subjects ORDER BY course_name",
        )
        ->fetchAll();
} catch (PDOException $e) {
}
        ...
        <div class="content-card">
            ...
            <form action="" method="GET" style="margin: 20px 0; display: flex; gap: 10px;">
                <input type="text" name="q" value="<?= htmlspecialchars(
                    $search,
                ) ?>" placeholder="Cari..." style="flex:1; padding: 10px; border:1px solid #e5e7eb; border-radius:8px;">
                <button type="submit" style="padding:10px 20px; background:#374151; color:white; border:none; border-radius:8px;">Cari</button>
            </form>
            <?php if (isset($message) && $message): ?>
                <div style="padding: 15px; margin-bottom: 20px; border-radius: 8px; background: <?= $message_type ===
                "success"
                    ? "#dcfce7"
                    : "#fee2e2" ?>; color: <?= $message_type === "success"
    ? "#15803d"
    : "#b91c1c" ?>;"><?= htmlspecialchars($message) ?></div>
            <?php endif; ?>
                    ...
                    <tbody>
                        <?php foreach ($users as $u): ?>
                            <tr style="border-bottom: 1px solid #e5e7eb;">
                                ...
                                <td style="padding: 12px;"><?= strtoupper(
                                    $u["role"],
                                ) ?></td>
     ...
     $u["detail"],
                                ...
                                <td style="padding: 12px; text-align: right;">
                                    <button class="action-btn" onclick="openEditModal(<?= htmlspecialchars(
                                        json_encode($u),
                                    ) ?>)">Edit</button>
                                    <form action="" method="POST" style="display: inline;" onsubmit="return confirm("Hapus?")">
                                        <input type="hidden" name="action" value="delete"><input type="hidden" name="user_id" value="<?= $u[
                                            "id"
                                        ] ?>">
...
<div id="editModal" class="modal-overlay">
            ...
            <div class="modal-body">
                ...
                <div id="edit_lecturer" style="display:none; flex-direction:column; gap:10px;">
                    ...
                    <label>Mata Kuliah Diampu
                        <select name="assigned_courses[]" id="edit_courses" multiple style="height: 100px;">
    ...
    $subj["course_code"] . " - " . $subj["course_name"],
                        ...
                        <span style="font-size:0.7rem; color:#6b7280; font-weight:normal;">Tahan Ctrl/Cmd untuk memilih lebih dari satu.</span>
                    </label>
                </div>
...
function openEditModal(u) {
    ...
    const courseSelect = document.getElementById("edit_courses");
    for (let i = 0; i < courseSelect.options.length; i++) {
        courseSelect.options[i].selected = false;
    }
    if (u.assigned_courses) {
        const assigned = u.assigned_courses.split(",");
        for (let i = 0; i < courseSelect.options.length; i++) {
            if (assigned.includes(courseSelect.options[i].value)) {
